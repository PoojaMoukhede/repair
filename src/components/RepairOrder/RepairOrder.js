...
export default function RepairOrder() {
  ...
  const [nextInvoiceNumber, seteNxtInvoiceNumber] = useState("");
  ...
  const handleSubmitlast = async (e) => {
    ...
    const shippingData = {
      ...
      // invoice_number:'MCIPL/RPR/2425/00027',
  ...
  console.log(`shipping data : ${invoice}`);
  useEffect(() => {
    try {
      axios
  ...
  useEffect(() => {
    if (lastInvoiceNumber !== null && lastInvoiceNumber !== undefined) {
      ...
      if (!isNaN(lastInvoiceNumberNumeric)) {
        ...
        const formattedNumber = nextNumber.toString().padStart(5, "0");
        const newNextInvoiceNumber = `MCIPL/RPR/${currentYear.toString().substring(2)}${(currentYear + 1).toString().substring(2)}/${formattedNumber}`;
          seteNxtInvoiceNumber(newNextInvoiceNumber);
      } else {
        console.error(
          "Failed to extract numeric part from the last Invoice number."
        );
      }
    } else {
      console.error(
        "Failed to generate Invoice number. Last Invoice number not available."
      );
    }
  }, [lastInvoiceNumber]);
